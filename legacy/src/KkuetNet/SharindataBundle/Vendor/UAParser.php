...
class UAParser {
	...
	*/
	public static function parse($ua = NULL, $accept= NULL, $wap_profile = NULL, $profile = NULL) {
		...
		if (!$result) {
			if ((strpos(self::$accept,'text/vnd.wap.wml') > 0) || (strpos(self::$accept,'application/vnd.wap.xhtml+xml') > 0) || isset($wap_profile) || isset($profile)) {
				$result = new \stdClass();
